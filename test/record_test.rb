...
class Proxy::DHCPRecordTest < Test::Unit::TestCase
  ...
  def setup
    ...
    options = {}
    @record = Proxy::DHCP::Record.new(@subnet, @ip, @mac, options)
  end
  ...
  def test_should_convert_to_string
    ...
    assert_equal Proxy::DHCP::Record.new(@subnet, ip, mac).to_s, "#{ip} / #{mac}"
  end
  ...
  def test_should_not_save_invalid_ip_addresses
    ...
    assert_raise Proxy::Validations::Error do
      Proxy::DHCP::Record.new(@subnet, ip, @mac)
    end
  end
  ...
  def test_mac_should_be_saved_lower_case
    ...
    ip = "192.168.0.12"
    assert_equal Proxy::DHCP::Record.new(@subnet, ip, mac).mac, mac.downcase
  end
